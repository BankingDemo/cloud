...
function getQueryVariables(query) {
    ...
    console.log("vars len: " + vars.length);
    for (var i = 0; i < vars.length; i++) {
        var pair = vars[i].split('=');
        console.log("pair[0] " + decodeURIComponent(pair[0]) + " -- pair[1] " + decodeURIComponent(pair[1]));
...
function cloudRoute() {
    ...
    cloud.get('/getBackendData', function(req,res){
        activity.record({
            ...
            console.log("starting param handling");
            var method = req.query.method.toUpperCase();
            var restSpecifiedPath = req.query.restpath;
            
            console.log("method: " + method + " and restpath: " + restSpecifiedPath);
            
            //var id = req.query.id;
            console.log("query: " + JSON.stringify(req.query));
            //var map = getQueryVariables(req.query);
            delete req.query["method"];
            delete req.query["restpath"];
            delete req.query["__fh"];
            console.log("query after delete: " + JSON.stringify(req.query));
            
            var path = restBasePath + restSpecifiedPath;
            //console.log("map['method'] " + map["method"]);
            ...
            var restRequest = http.request(options, function(restResponse){
                ...
                restResponse.on("end", function () {
                    ...
                    console.log("resp json " + jsonData);
                    res.send(data);
                });
            });
